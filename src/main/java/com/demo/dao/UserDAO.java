...
public class UserDAO {
	...
	String insertUserSql = "INSERT INTO USER (ID, USERNAME, CITY) VALUES (SEQ_USER_IID.NEXTVAL , ? , ?)";
	...
	String deleteContentSql = "DELETE FROM RELATIONS WHERE CUBEID = ? AND CONTENTID = ?";
	String deleteCubeSql    = "DELETE FROM CUBE WHERE CUBEID = ? ";
	...
	public Long getUserId( String username , String city)
	{
		...
		try{
			String getUserSql = "SELECT * FROM USER WHERE USERNAME = ? AND CITY = ?";
		    id= jdbcTemplate.queryForLong(getUserSql , new Object[]
					{
					username , city
					});
	...
	public Long getCubeId( long id , String name)
	{
		...
		try{
			String getUserSql = "SELECT * FROM CUBE WHERE USEID = ? AND NAME = ?";
		    cubeId= jdbcTemplate.queryForLong(getUserSql , new Object[]
					{
					id , name
					});
	...
	public List<String> insertContent( long id , String link)
	{
		...
		Long contentId = getCubeId(id, link);
		String userId = Long.toString( id);
	...
	public Long getContentId( long id , String link)
	{
		...
		try{
			String getUserSql = "SELECT * FROM CONTENT WHERE USEID = ? AND LINK = ?";
			contentId= jdbcTemplate.queryForLong(getUserSql , new Object[]
					{
					id , link
					});
	...
	public Long getRelationId( long cubeId , long contentId)
	{
		...
		try{
			String getUserSql = "SELECT * FROM RELATION WHERE CUBEID = ? AND CONTENTID = ?";
			relationId= jdbcTemplate.queryForLong(getUserSql , new Object[]
					{
					cubeId , contentId
					});
	...
	public Long getShareCubeId( long userId , long cubeId)
	{
		...
		try{
			String getUserSql = "SELECT * FROM SHAREDCUBE WHERE USEID = ? AND CUBEID = ?";
			sharedId= jdbcTemplate.queryForLong(getUserSql , new Object[]
					{
					userId , cubeId
					});
	...
	public List<String> shareContent( long contentId , long userId1 , long userId2)
	{
		...
		try{
			Connection conn = getConnection();
				...
				shareCubeStmt.setLong(1, contentId);
				shareCubeStmt.setLong(2, userId2);
				shareCubeStmt.execute();
		}
		catch (SQLException e)
	...
	public Long getShareContentId( long userId , long contentId)
	{
		...
		try{
			String getSharedSqlId = "SELECT * FROM SHAREDCONTENT WHERE USEID = ? AND CUBEID = ?";
			sharedId= jdbcTemplate.queryForLong(getSharedSqlId , new Object[]
					{
					userId , contentId
					});
	...
	public List<Cube> getAllCube( long userId)
	{
		...
		try{
			String query = "SELECT SC.ID , C.NAME  FROM SHAREDCUBE SC , CUBE C WHERE  SC.CUBEID = C.CUBEID AND SC.USEID = ? ";
			List<Map<String, Object>> cubeList= jdbcTemplate.queryForList(query , new Object[]
					{
					userId 
					});
	...
	public List<Content> getAllContent( long userId)
	{
		...
		try{
			String query = "SELECT SC.ID , C.NAME  FROM SHAREDCONTENT SC , CONTENT C WHERE  SC.CONTENTID = C.CONTENTID AND SC.USEID = ? ";
			List<Map<String, Object>> contentList= jdbcTemplate.queryForList(query , new Object[]
					{
					userId 
					});
			String query1 = "SELECT SC.ID , C.NAME  FROM SHAREDCONTENT SC , CONTENT C , SHAREDCUBE SB , RELATION R WHERE  SC.CONTENTID = C.CONTENTID AND  SC.CONTENTID = R.CONTENTID AND R.CUBEID = SB.CUBEID AND  SB.USEID = ? ";
			List<Map<String, Object>> contentList1= jdbcTemplate.queryForList(query1 , new Object[]
					{
					userId 
					});
	...
	public boolean isUserCubeLink(long userId , long cubeId)
	{
		...
		try
		{
			...
			long num = jdbcTemplate.queryForLong(query , new Object[] 
					{
					cubeId , userId
					});
			if( num >0)
	...
	public boolean isUserContentLink(long userId , long contentId)
	{
		...
		try
		{
			String query = "SELECT COUNT(*) FROM SHAREDCONTENT WHERE CONTENT = ? AND USERID = ?";
			long num = jdbcTemplate.queryForLong(query , new Object[] 
					{
					contentId , userId
					});
